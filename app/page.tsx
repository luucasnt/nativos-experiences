...
export default function Home() {
  ...
  const homeFaqs = [serviceData.airport.faqs[0], serviceData.airport.faqs[1], serviceData.fleet.faqs[1], serviceData.transfer.faqs[2]];
  ...
  return <main id="top" className="site-shell sand-theme">
    ...
    <section className="experiences section-light" aria-labelledby="routes-title"><div className="section-head reveal"><div><p className="eyebrow dark"><span className="eyebrow-dot" /> Rotas mais procuradas</p><h2 id="routes-title">Do desembarque<br /><em>ao destino certo.</em></h2></div><p className="section-note">Chegadas por Porto Seguro ou Terravista e deslocamentos privativos pela região, planejados conforme passageiros, bagagens, horário e hospedagem.</p></div><div className="experience-list">{routes.map(([from,to,href],i) => <a className="experience-card reveal" href={href} key={`${from}-${to}`}><div className="experience-info"><div><p className="card-region">ROTA {String(i+1).padStart(2,"0")}</p><h3>{from}<br />→ {to}</h3></div><p className="card-description">Recepção e deslocamento privativo com planejamento da operação.<br /><strong>Ver rota →</strong></p></div></a>)}</div></section>
    ...
    <section id="frota" className="fleet-section section-dark" aria-labelledby="fleet-title"><div className="section-index">N / 04</div><div className="fleet-head reveal"><p className="eyebrow light"><span className="eyebrow-dot" /> A categoria certa para cada viagem</p><h2 id="fleet-title">Escolha o padrão.<br /><em>Nós cuidamos do restante.</em></h2><p>De categorias compactas a SUVs premium, vans e Mercedes-Benz GLE 400d blindado. A categoria é definida conforme passageiros, bagagens, trajeto e nível de conforto; o modelo de referência pode variar conforme disponibilidade.</p><a className="hero-action" href="/frota">Conhecer categorias</a></div></section>
