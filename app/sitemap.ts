...
const guideEntry = (slug: string): MetadataRoute.Sitemap[number] => ({ url: `${siteUrl}/guia/${slug}`, lastModified: new Date(), changeFrequency: "monthly", priority: .7 });

export default function sitemap(): MetadataRoute.Sitemap { return [entry("", "pt-BR"), entry("", "en"), ...canonicalSlugs.flatMap((slug) => [entry(slug, "pt-BR"), entry(slug, "en")]), { url: `${siteUrl}/guia`, lastModified: new Date(), changeFrequency: "weekly", priority: .75 }, ...guideSlugs.map(guideEntry)]; }
